...
import com.pdfcreator.signature.*;
import com.pdfcreator.rasterizer.*;
import com.pdfcreator.validator.*;
import com.pdfcreator.pdfa.*;
...
public class PdfCreatorTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Files.createDirectories(Path.of(OUT_DIR + "/signature"));
        Files.createDirectories(Path.of(OUT_DIR + "/renderer"));
        ...
        test36_signature_generateKeystore();
        test37_signature_sign_invisible();
        test38_signature_verify_valid();
        test39_signature_verify_tampered();
        test40_signature_listSignatures();
        test41_signature_exportCert();
        test42_rasterize_allPagesAsPng();
        test43_rasterize_singlePageAsJpeg();
        test44_rasterize_pageRange();
        test45_validate_nonPdfA();
        test46_generate_pdfA_compliant();
        test47_validate_encrypted_fails();
        test48_merge_retains_pdfA();
        test49_split_retains_pdfA();
        test50_validate_pdfA_passes();
    ...
    // -----------------------------------------------------------------------
    // TEST 36 — Generate a self-signed PKCS12 keystore for subsequent tests
    //           Uses keytool (must be on PATH — part of any JDK installation).
    //           Skips gracefully if keytool is not available.
    // -----------------------------------------------------------------------
    static void test36_signature_generateKeystore() {
        run("36 Signature — generate test keystore (keytool)", () -> {
            String keystorePath = OUT_DIR + "/signature/test-keystore.p12";

            // Skip if already generated by a previous run
            ...
            ProcessBuilder pb = new ProcessBuilder(
                ...
                "-keyalg", "RSA",
                "-keysize", "2048",
                "-dname", "CN=PdfCreator Test, OU=Dev, O=TestOrg, L=Chennai, ST=TN, C=IN",
            ...
            try {
                ...
                System.out.println("      keytool not found — skipping keystore generation.");
                System.out.println("      Remaining signature tests will also be skipped.");
                return;
            }
            ...
            try {
                ...
                throw new RuntimeException("keytool interrupted", e);
            }

            String output = new String(proc.getInputStream().readAllBytes());
            if (exit != 0)
                throw new AssertionError("keytool failed (exit " + exit + "): " + output);

            assertFileValid(keystorePath);
            System.out.printf("      Keystore generated: %s%n", keystorePath);
        });
    }

    // -----------------------------------------------------------------------
    // TEST 37 — Sign a PDF with an invisible signature
    //           Uses the keystore from test 36. Verifies the output file is
    //           larger than the input (signature bytes were appended).
    // -----------------------------------------------------------------------
    static void test37_signature_sign_invisible() {
        run("37 Signature — sign PDF (invisible)", () -> {
            String keystorePath = OUT_DIR + "/signature/test-keystore.p12";
            if (!new java.io.File(keystorePath).exists()) {
                System.out.println("      Skipped — keystore not available (test 36 skipped).");
                return;
            }

            // Generate a source PDF if not present
            String source = OUT_DIR + "/01-direct-inline.pdf";
            if (!new java.io.File(source).exists()) {
                PdfCreator.main(new String[]{
                    "--title", "Signature Test",
                    "--text",  "This document will be digitally signed.",
                    "--output", source
                });
            }
            assertFileValid(source);
            long unsignedSize = new java.io.File(source).length();

            String output = OUT_DIR + "/signature/37-signed-invisible.pdf";
            ...
            SigningOptions opts = new SigningOptions.Builder()
                ...
                .contactInfo("test@pdfcreator.com")
                .build();

            SignatureResult result = signer.sign(opts);

            assertFileValid(output);
            if (result.getOutputSizeBytes() <= unsignedSize)
                throw new AssertionError(
                    "Signed file (" + result.getOutputSizeBytes() + " bytes) should be " +
                    "larger than unsigned file (" + unsignedSize + " bytes)");

            System.out.printf("      Signed: %s%n", output);
            System.out.printf("      Signer: %s%n", result.getSignerName());
            System.out.printf("      Size: unsigned=%d, signed=%d bytes%n",
                unsignedSize, result.getOutputSizeBytes());
        });
    }

    // -----------------------------------------------------------------------
    // TEST 38 — Verify the signed PDF — expects signatureValid=true
    //           Uses the PDF signed in test 37.
    // -----------------------------------------------------------------------
    static void test38_signature_verify_valid() {
        run("38 Signature — verify signed PDF (expect valid)", () -> {
            String signedPdf    = OUT_DIR + "/signature/37-signed-invisible.pdf";
            String keystorePath = OUT_DIR + "/signature/test-keystore.p12";
            String certPath     = OUT_DIR + "/signature/test-cert.cer";

            if (!new java.io.File(signedPdf).exists()) {
                System.out.println("      Skipped — signed PDF not available (test 37 skipped).");
                return;
            }

            // Export cert first (needed for trust anchor)
            ...
            assertFileValid(certPath);

            // Verify using the exported cert as trust anchor
            PdfSignatureVerifier verifier = new PdfSignatureVerifier();
            java.util.List<VerificationResult> results =
                verifier.verify(signedPdf, certPath, null, "CER");

            if (results.isEmpty())
                throw new AssertionError("Expected at least 1 signature, found none");

            VerificationResult r = results.get(0);

            if (!r.isSignatureValid())
                throw new AssertionError(
                    "Expected signatureValid=true but got false. Reason: "
                    + r.getFailureReason());

            if (!r.isCoverageComplete())
                throw new AssertionError("Expected coverageComplete=true");

            System.out.printf("      signatureValid=%b, trusted=%b, coverage=%b%n",
                r.isSignatureValid(), r.isCertTrusted(), r.isCoverageComplete());
            System.out.printf("      Signer: %s%n", r.getSignerName());
        });
    }

    // -----------------------------------------------------------------------
    // TEST 39 — Tamper with a signed PDF and verify — expects signatureValid=false
    //           Flips one byte in the content area of the signed PDF, then
    //           verifies that the digest mismatch is detected.
    // -----------------------------------------------------------------------
    static void test39_signature_verify_tampered() {
        run("39 Signature — verify tampered PDF (expect invalid)", () -> {
            String signedPdf   = OUT_DIR + "/signature/37-signed-invisible.pdf";
            String tamperedPdf = OUT_DIR + "/signature/39-tampered.pdf";
            String certPath    = OUT_DIR + "/signature/test-cert.cer";
            ...
            // Copy the signed PDF and flip a byte near the middle (inside the content)
            byte[] bytes = java.nio.file.Files.readAllBytes(java.nio.file.Path.of(signedPdf));
            int tamperPos = bytes.length / 2;
            bytes[tamperPos] = (byte)(bytes[tamperPos] ^ 0x01);  // flip one bit
            java.nio.file.Files.write(java.nio.file.Path.of(tamperedPdf), bytes);
            assertFileValid(tamperedPdf);

            // Verify — should report signatureValid=false
            PdfSignatureVerifier verifier = new PdfSignatureVerifier();
            java.util.List<VerificationResult> results;
            try {
                results = verifier.verify(tamperedPdf, certPath, null, "CER");
            } catch (Exception e) {
                // Some tamper positions corrupt the PDF structure — that's also a "failed" verify
                System.out.printf("      Tampered PDF caused parse error (expected): %s%n",
                    e.getClass().getSimpleName());
                return;
            }

            if (results.isEmpty()) {
                System.out.println("      Tampered PDF: no signatures parseable (expected for severe tampering).");
                return;
            }
            ...
            if (r.isSignatureValid())
                throw new AssertionError(
                    "Expected signatureValid=false for tampered PDF but got true");

            System.out.printf("      Tamper detected correctly: signatureValid=%b%n",
                r.isSignatureValid());
            if (r.getFailureReason() != null)
                System.out.printf("      Failure reason: %s%n", r.getFailureReason());
        });
    }

    // -----------------------------------------------------------------------
    // TEST 40 — List signatures without full verification
    //           Verifies signer name, reason, and location are reported.
    // -----------------------------------------------------------------------
    static void test40_signature_listSignatures() {
        run("40 Signature — list signatures metadata", () -> {
            String signedPdf = OUT_DIR + "/signature/37-signed-invisible.pdf";
            ...
            PdfSignatureVerifier verifier = new PdfSignatureVerifier();
            java.util.List<VerificationResult> results = verifier.listSignatures(signedPdf);

            if (results.isEmpty())
                throw new AssertionError("Expected at least 1 signature listing, found none");

            VerificationResult r = results.get(0);

            // Reason and location should be set from signing options
            if (!"Integration Test Signature".equals(r.getReason()))
                throw new AssertionError("Expected reason 'Integration Test Signature', got: "
                    + r.getReason());

            if (!"Chennai, India".equals(r.getLocation()))
                throw new AssertionError("Expected location 'Chennai, India', got: "
                    + r.getLocation());

            System.out.printf("      Found %d signature(s)%n", results.size());
            System.out.printf("      Signer  : %s%n", r.getSignerName());
            System.out.printf("      Reason  : %s%n", r.getReason());
            System.out.printf("      Location: %s%n", r.getLocation());
        });
    }

    // -----------------------------------------------------------------------
    // TEST 41 — Export public certificate from keystore
    //           Verifies the .cer file is created and is a valid DER cert.
    // -----------------------------------------------------------------------
    static void test41_signature_exportCert() {
        run("41 Signature — export certificate from keystore", () -> {
            String keystorePath = OUT_DIR + "/signature/test-keystore.p12";
            String certOutput   = OUT_DIR + "/signature/41-exported.cer";
            ...
            assertFileValid(certOutput);

            // Verify it parses as a valid X.509 certificate
            java.security.cert.CertificateFactory cf =
                java.security.cert.CertificateFactory.getInstance("X.509");
            java.security.cert.X509Certificate cert;
            try (java.io.FileInputStream fis = new java.io.FileInputStream(certOutput)) {
                cert = (java.security.cert.X509Certificate) cf.generateCertificate(fis);
            }

            if (cert == null)
                throw new AssertionError("Failed to parse exported certificate");

            String cn = cert.getSubjectX500Principal().getName();
            if (!cn.contains("PdfCreator Test"))
                throw new AssertionError("Expected CN=PdfCreator Test in cert, got: " + cn);

            System.out.printf("      Certificate exported: %s%n", certOutput);
            System.out.printf("      Subject: %s%n", cn);
    ...
    // -----------------------------------------------------------------------
    // TEST 42 — Rasterize all pages of a PDF to PNG at 150 DPI
    //           Verifies the correct number of image files are created and
    //           each is a non-empty PNG file.
    // -----------------------------------------------------------------------
    static void test42_rasterize_allPagesAsPng() {
        run("42 Rasterize — all pages to PNG (150 DPI)", () -> {
            // Use the bank-statement template output from test04 if available,
            // otherwise fall back to the simplest direct-mode output.
            String source = OUT_DIR + "/04-bank-statement-cust001.pdf";
            if (!new java.io.File(source).exists())
                source = OUT_DIR + "/01-direct-inline.pdf";
            assertFileValid(source);

            String outDir = OUT_DIR + "/renderer/42-png";
            ...
            if (result.getPagesRendered() <= 0)
                throw new AssertionError("Expected at least 1 page rendered, got 0");

            if (result.getOutputFiles().size() != result.getPagesRendered())
                throw new AssertionError("Output file count mismatch: files="
                    + result.getOutputFiles().size()
                    + " pagesRendered=" + result.getPagesRendered());

            // Every file must exist, be non-empty, and end with .png
            for (String f : result.getOutputFiles()) {
                java.io.File img = new java.io.File(f);
                if (!img.exists() || img.length() == 0)
                    throw new AssertionError("Output image missing or empty: " + f);
                if (!f.toLowerCase().endsWith(".png"))
                    throw new AssertionError("Expected .png extension but got: " + f);
            }

            System.out.printf("      Rendered %d page(s) → %s%n",
                result.getPagesRendered(), outDir);
            System.out.printf("      Total size: %,d bytes, %.2fs%n",
                result.getTotalSizeBytes(), result.getDurationMs() / 1000.0);
        });
    }

    // -----------------------------------------------------------------------
    // TEST 43 — Rasterize page 1 to JPEG at 200 DPI
    //           Verifies the output is a .jpg file and JPEG quality is applied.
    // -----------------------------------------------------------------------
    static void test43_rasterize_singlePageAsJpeg() {
        run("43 Rasterize — single page to JPEG (200 DPI, quality 0.90)", () -> {
            String source = OUT_DIR + "/01-direct-inline.pdf";
            assertFileValid(source);

            String outDir = OUT_DIR + "/renderer/43-jpeg";
            ...
            if (result.getPagesRendered() != 1)
                throw new AssertionError("Expected 1 page rendered, got: "
                    + result.getPagesRendered());

            String outFile = result.getOutputFiles().get(0);
            if (!outFile.toLowerCase().endsWith(".jpg"))
                throw new AssertionError("Expected .jpg extension but got: " + outFile);

            java.io.File img = new java.io.File(outFile);
            if (!img.exists() || img.length() == 0)
                throw new AssertionError("JPEG output missing or empty: " + outFile);

            // Filename should use the custom prefix
            if (!img.getName().startsWith("thumbnail"))
                throw new AssertionError("Expected filename to start with 'thumbnail', got: "
                    + img.getName());

            System.out.printf("      JPEG: %s (%,d bytes)%n", img.getName(), img.length());
        });
    }

    // -----------------------------------------------------------------------
    // TEST 44 — Rasterize a page range (CLI path)
    //           Exercises the CLI routing and verifies only the requested
    //           pages are rendered.
    // -----------------------------------------------------------------------
    static void test44_rasterize_pageRange() {
        run("44 Rasterize — page range via CLI (PNG, 72 DPI)", () -> {
            String source = OUT_DIR + "/04-bank-statement-cust001.pdf";
            if (!new java.io.File(source).exists()) {
                System.out.println("      Skipped — bank statement not available.");
                return;
            }

            // Check how many pages the source has
            int totalPages;
            try (org.apache.pdfbox.pdmodel.PDDocument doc =
                     org.apache.pdfbox.Loader.loadPDF(new java.io.File(source))) {
                totalPages = doc.getNumberOfPages();
            }

            if (totalPages < 1) {
                System.out.println("      Skipped — source PDF has no pages.");
                return;
            }

            String outDir = OUT_DIR + "/renderer/44-range";
            int endPage = Math.min(2, totalPages);  // render up to 2 pages
            ...
            PdfCreator.main(new String[]{
                ...
                "--dpi",        "72",
                "--start-page", "1",
                "--end-page",   String.valueOf(endPage),
                "--file-prefix","slide"
            });

            // Verify the correct number of files were created
            java.io.File[] files = new java.io.File(outDir)
                .listFiles((d, n) -> n.endsWith(".png"));
            if (files == null || files.length != endPage)
                throw new AssertionError("Expected " + endPage + " PNG file(s), found: "
                    + (files == null ? 0 : files.length));

            System.out.printf("      Created %d PNG file(s) in %s%n", files.length, outDir);
        });
    }


    // -----------------------------------------------------------------------
    // TEST 45 — Validate a standard (non-PDF/A) PDF
    //           Expects isValid=false with font and metadata errors
    // -----------------------------------------------------------------------
    static void test45_validate_nonPdfA() {
        run("45 Validate — non-PDF/A file (expect failures)", () -> {
            String source = OUT_DIR + "/01-direct-inline.pdf";
            assertFileValid(source);

            PdfValidationResult result = new PdfValidator().validate(
            ...
            // A standard PDFBox-generated PDF with Standard14 fonts MUST fail
            if (result.isValid())
                throw new AssertionError("Expected validation to fail for non-PDF/A document");

            System.out.printf("      Found %d issue(s)%n", result.getIssueCount());
            result.getCountByCategory().forEach((cat, count) ->
                System.out.printf("        %-25s %d%n", cat + ":", count));

            // Must have font-related errors (Standard14 fonts are not embedded)
            boolean hasFontError = result.getIssues().stream()
                .anyMatch(i -> i.getErrorCode().startsWith("3"));
            if (!hasFontError)
                throw new AssertionError("Expected font embedding errors (code 3.x) but none found");
        });
    }

    // -----------------------------------------------------------------------
    // TEST 46 — Generate a PDF/A-1b compliant document
    //           Exercises embedded fonts + XMP + output intent
    // -----------------------------------------------------------------------
    static void test46_generate_pdfA_compliant() {
        run("46 Generate PDF/A-1b compliant document", () -> {
            String templateFile = "templates/pdf-templates.json";
            String configFile   = "configs/pdf-configs.json";
            String dataFile     = "data/statements/statement-001.json";
            String outputPath   = OUT_DIR + "/validator/46-pdfa-compliant.pdf";

            // Skip if template files not present
            if (!new java.io.File(templateFile).exists() ||
                !new java.io.File(dataFile).exists()) {
                System.out.println("      Skipped — template/data files not available.");
                return;
            }

            new com.pdfcreator.pipeline.RenderPipeline(templateFile, configFile)
                .withPdfA(true)
                .render("bank-statement",
                    new com.pdfcreator.datasource.JsonFileDataSource(dataFile),
                    outputPath);

            assertFileValid(outputPath);

            // Quick check: the file must be larger than without PDF/A
            // (embedded fonts add bytes)
            long size = new java.io.File(outputPath).length();
            System.out.printf("      PDF/A output: %,d bytes%n", size);
            if (size < 10_000)
                throw new AssertionError("PDF/A output suspiciously small: " + size + " bytes");
        });
    }

    // -----------------------------------------------------------------------
    // TEST 47 — Validate an encrypted PDF
    //           Encryption is forbidden by PDF/A-1b — must fail
    // -----------------------------------------------------------------------
    static void test47_validate_encrypted_fails() {
        run("47 Validate — encrypted PDF (expect encryption error)", () -> {
            String source = OUT_DIR + "/security/30-encrypted-all.pdf";
            if (!new java.io.File(source).exists()) {
                System.out.println("      Skipped — encrypted PDF not available.");
                return;
            }
            ...
            if (result.isValid())
                throw new AssertionError("Expected encrypted PDF to fail PDF/A validation");

            System.out.printf("      %d issue(s) found as expected%n", result.getIssueCount());

            // Should have a structure/encryption error (code 8.x)
            boolean hasEncryptionError = result.getIssues().stream()
                .anyMatch(i -> i.getErrorCode().startsWith("8") ||
                               i.getDetail().toLowerCase().contains("encrypt"));
            // Encryption error OR parse failure — either is acceptable
            System.out.println("      Categories: " + result.getCountByCategory());
        });
    }

    // -----------------------------------------------------------------------
    // TEST 48 — Merge two PDFs; if source was PDF/A, output retains markers
    // -----------------------------------------------------------------------
    static void test48_merge_retains_pdfA() {
        run("48 Merge — PDF/A compliance transferred to merged output", () -> {
            String pdfA = OUT_DIR + "/validator/46-pdfa-compliant.pdf";
            String regular = OUT_DIR + "/01-direct-inline.pdf";
            String merged  = OUT_DIR + "/validator/48-merged.pdf";

            if (!new java.io.File(pdfA).exists() ||
                !new java.io.File(regular).exists()) {
                System.out.println("      Skipped — source PDFs not available.");
                return;
            }
            ...
            assertFileValid(merged);

            // Merged output should have PDF/A markers because one input was PDF/A
            try (org.apache.pdfbox.pdmodel.PDDocument doc =
                     org.apache.pdfbox.Loader.loadPDF(new java.io.File(merged))) {
                boolean isPdfA = PdfACompliance.isPdfA(doc);
                System.out.printf("      Merged PDF/A markers present: %b%n", isPdfA);
                // This is a best-effort check — we log but don't fail if not present
            }
        });
    }

    // -----------------------------------------------------------------------
    // TEST 49 — Split a PDF/A document; each part retains PDF/A markers
    // -----------------------------------------------------------------------
    static void test49_split_retains_pdfA() {
        run("49 Split — PDF/A compliance transferred to split parts", () -> {
            String source  = OUT_DIR + "/validator/46-pdfa-compliant.pdf";
            String outDir  = OUT_DIR + "/validator/49-split";

            if (!new java.io.File(source).exists()) {
                System.out.println("      Skipped — PDF/A source not available.");
                return;
            }
            ...
            java.io.File[] parts = new java.io.File(outDir)
                .listFiles((d, n) -> n.endsWith(".pdf"));
            if (parts == null || parts.length == 0)
                throw new AssertionError("No split parts produced");

            System.out.printf("      %d part(s) produced%n", parts.length);
            // Check first part for PDF/A markers
            try (org.apache.pdfbox.pdmodel.PDDocument part =
                     org.apache.pdfbox.Loader.loadPDF(parts[0])) {
                boolean isPdfA = PdfACompliance.isPdfA(part);
                System.out.printf("      Part[0] PDF/A markers: %b%n", isPdfA);
            }
        });
    }

    // -----------------------------------------------------------------------
    // TEST 50 — Validate the PDF/A document generated in test 46
    //           Should pass or have only minor issues
    // -----------------------------------------------------------------------
    static void test50_validate_pdfA_passes() {
        run("50 Validate — generated PDF/A-1b document", () -> {
            String source = OUT_DIR + "/validator/46-pdfa-compliant.pdf";
            if (!new java.io.File(source).exists()) {
                System.out.println("      Skipped — PDF/A file not available.");
                return;
            }
            ...
            System.out.printf("      Valid: %b  Issues: %d%n",
                result.isValid(), result.getIssueCount());
            if (!result.getIssues().isEmpty()) {
                System.out.println("      Issue categories: " + result.getCountByCategory());
                result.getIssues().stream().limit(5).forEach(i ->
                    System.out.println("        " + i));
            }
            // We log results but don't hard-fail — some minor issues may remain
            // depending on content (e.g. colour space handling for images).
            // The important thing is the document opens and XMP markers are present.
            if (result.isValid()) {
                System.out.println("      ✓ FULLY COMPLIANT");
            } else {
                System.out.println("      ~ Partial compliance — see issues above");
            }
        });
    }
